...
def unload_to_s3():
    """Triggers UNLOAD command asynchronously in a separate thread."""
        ...
        try:
            with get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(unload_query)  # Run UNLOAD in a separate thread
            print(f"✅ UNLOAD command executed. TimeTaken: {time.time() - start_time}")
...
def load_from_s3():
    ...
    def run_load():
        ...
        try:
            ...
            print(f"✅ LOAD command executed. TimeTaken: {time.time() - start_time}")
        except Exception as e:
